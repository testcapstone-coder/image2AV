...
VIDEO_MODEL = "Wan-AI/Wan2.1-T2V-1.3B"  # Optional story -> short video via HF Inference Providers
...
VIDEO_STYLES = {
    "📖 Storybook": (
        "children's storybook illustration brought to life, whimsical, soft colors, "
        "warm gentle lighting, charming hand-crafted details, smooth subtle movement"
    ),
    "🧸 3D Cartoon": (
        "charming 3D animated children's movie style, expressive friendly characters, "
        "bright colorful environment, soft cinematic lighting, playful smooth movement"
    ),
    "🎨 Watercolor": (
        "hand-painted watercolor children's book style, pastel colors, visible brush textures, "
        "dreamy soft edges, delicate gentle animation"
    ),
    "🎞️ Cinematic": (
        "family-friendly cinematic scene, detailed environment, natural warm lighting, "
        "soft depth of field, smooth camera movement, polished film look"
    ),
}

VIDEO_NEGATIVE_PROMPT = (
    "blurry, low quality, distorted face, deformed hands, extra fingers, extra limbs, "
    "frightening imagery, violence, injury, adult content, text, subtitles, captions, "
    "logos, watermark, flickering, inconsistent characters"
)
...
def build_video_prompt(story: str, description: str, style_name: str) -> str:
    """Turn the finished story into a concise, child-friendly video prompt."""
    style = VIDEO_STYLES[style_name]
    return (
        "Create one short, joyful, child-friendly video inspired by the story below. "
        "Show one coherent visual moment rather than trying to depict every sentence. "
        "Preserve the main subject, setting, colors, objects, and visual details from the image description. "
        "Use gentle natural movement and keep characters visually consistent. "
        "Do not add text, subtitles, logos, or frightening content.\\n\\n"
...
def generate_video(story: str, description: str, style_name: str) -> bytes:
    """Generate a short video remotely with Hugging Face Inference Providers."""
    token = get_hf_token()
    if not token:
        raise RuntimeError(
            "Video generation needs a Hugging Face token. Add HF_TOKEN to Streamlit "
...
def render_video_section(result: dict):
    """Show optional video generation only after narration is available."""
    if not result.get("audio") or not result.get("story"):
        return

    with st.container(border=True, key="video_panel"):
        st.markdown(
            """
            <div class="section-kicker">Going further</div>
            <div class="section-title">See Your Story</div>
            <div class="section-copy">
                Turn your finished story into a short video. Choose a visual style,
                then let the story come to life.
            </div>
            """,
            unsafe_allow_html=True,
        )

        style_col, action_col = st.columns([1.25, 1], gap="large")

        with style_col:
            selected_style = st.selectbox(
                "Choose a video style",
                list(VIDEO_STYLES.keys()),
                key="video_style",
            )

        token_available = bool(get_hf_token())

        with action_col:
            st.markdown(
                '<div class="control-heading">Create your video</div>',
                unsafe_allow_html=True,
            )
            create_video = st.button(
                "🎬 Generate Video",
                type="primary",
                use_container_width=True,
                disabled=not token_available,
                key="generate_video",
            )

        if not token_available:
            st.info(
                "To enable video generation, add an `HF_TOKEN` secret with Hugging Face "
                "Inference Providers access in Streamlit Cloud."
            )

        if create_video:
            # Remove an older video before starting a new style.
            result.pop("video", None)
            result.pop("video_style", None)

            progress = st.progress(10, text="Preparing your story for video…")
            try:
                progress.progress(30, text="Sending your story to the video model…")
                with st.spinner("Creating your short video — this can take a few minutes…"):
                ...
                result["video"] = video
                result["video_style"] = selected_style
                st.session_state["result"] = result
                progress.progress(100, text="Your video is ready.")
        ...
        if video:
            st.markdown("### Watch your story")
            st.caption(f"Video style: {result.get('video_style', selected_style)}")
            ...
            if result.get("video_style") != selected_style:
                st.caption("Choose Generate Video again to apply the newly selected style.")
...
def inject_apple_style():
    ...
    st.markdown(
        ...
        /* Optional video experience shown only after narration is ready. */
        .st-key-video_panel {
            margin-top: 2.2rem !important;
        }
        .st-key-video_panel > div[data-testid="stVerticalBlockBorderWrapper"] {
            background:
                radial-gradient(circle at 10% 0%, rgba(100,168,255,.12), transparent 34%),
                radial-gradient(circle at 88% 100%, rgba(210,140,255,.11), transparent 36%),
                linear-gradient(145deg, rgba(19,20,29,.98), rgba(12,12,17,.99)) !important;
            border-color: rgba(167,139,250,.18) !important;
        }
        .st-key-video_panel [data-testid="stVideo"] {
            margin-top: .6rem;
            border-radius: 20px;
            overflow: hidden;
        }
...
def main():
    ...
    # Optional "Going further" experience appears only after narration exists.
    final_result = st.session_state.get("result")
    if final_result and final_result.get("audio"):
        render_video_section(final_result)
